...
- PySide6/PyQt6 双 Qt 绑定兼容
- lspci/lsusb 可选增强和 `/sys`、`/proc` 回退
- 真机 PCI、显示连接器、磁盘、网卡、USB、音频和系统信息
- 设备搜索、扫描诊断横幅和长字段自适应详情表
...
- Python `>= 3.11`
- Linux：PyQt6 `>= 6.7` 或 PySide6 `>= 6.7`
- Windows/其他平台：PySide6 `>= 6.7`
- Linux 环境用于读取真实硬件信息
...
项目会按平台选择 Qt 绑定：Linux 默认使用 PyQt6，其他平台默认使用 PySide6。运行时也会在两者之间自动回退；如果系统已经提供其中一个绑定，可以直接从源码启动。
...
左侧筛选框可以按设备名称、厂商、型号、驱动、位置或硬件 ID 搜索。扫描完成后，顶部横幅会显示设备数量、警告数量和读取提示；将鼠标悬停在提示横幅上可以查看具体失败原因。
...
- `/sys/class/input`
- `/sys/bus`
- 可选的 `lspci -D -mm -nn`：补充 PCI 厂商、型号、类别和子系统名称
- 可选的 `lsusb`：补充 USB 总线地址、VID/PID 和产品名称
- `/proc/asound`：补充 ALSA 声卡和 Codec 信息

程序会尽量读取可用字段。单个设备或单类设备读取失败时，会保留其他结果，并在扫描横幅和状态栏提示错误数量；`lspci`、`lsusb` 不存在时会自动回退到 `/sys` 和 `/proc`。
...
- 缺失路径和异常 Provider；
- 后台刷新并发保护和结果排序；
- lspci/lsusb 文本解析和 PCI 分类；
- 设备树筛选、详情面板长字段和 Qt 绑定兼容。
